...
<?= tep_draw_form('advanced_search', tep_href_link('advanced_search_result.php', '', 'NONSSL', false), 'get', 'onsubmit="return check_form(this);"') . tep_hide_session_id() ?>
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-9">
      <?= tep_draw_pull_down_menu('categories_id', $category_tree->get_selections([['id' => '', 'text' => TEXT_ALL_CATEGORIES]]), null, 'id="entryCategories"') ?>
    </div>
  </div>
  <div class="form-group row">
    ...
    <div class="col-sm-9">
      <div class="checkbox">
        <label>
          <?= tep_draw_checkbox_field('inc_subcat', '1', true, 'id="entryIncludeSubs"') ?>
        </label>
      </div>
    </div>
  </div>

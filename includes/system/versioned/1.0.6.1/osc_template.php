  ...
  class oscTemplate {
    ...
    public function __construct($template = 'default') {
      if (is_object($template)) {
        ...
        require DIR_FS_CATALOG . "templates/$template/includes/template." . pathinfo($GLOBALS['PHP_SELF'], PATHINFO_EXTENSION);
        $template .= '_template';
        $this->_template = new $template();
      }

      if (is_callable([$this->_template, 'autoload_hooks'])) {
        spl_autoload_register([$this->_template, 'autoload_hooks'], true, true);
      }
    }
    ...
    public function map_to_template($file, $type = 'module') {
      return $this->_template->get_template_mapping_for($file, $type) ?? $this->_map_to_template($file, $type);
    }

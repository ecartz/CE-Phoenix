...
EOSQL
    ...
    protected function _buildBranch($parent_id, $level = 0) {
      ...
      foreach ( $this->_data[$parent_id] ?? [] as $category_id => $category ) {
        ...
        if ( isset($this->_data[$category_id])
          ...
          && ( ( $this->follow_cpath !== true ) || in_array($category_id, $this->cpath_array) ) )
        {
    ...
    function buildBranchArray($parent_id, $level = 0, $result = []) {
      foreach ($this->_data[$parent_id] as $category_id => $category) {
        $category_link = $this->breadcrumb_usage
                       ? $this->buildBreadcrumb($category_id)
                       : $category_id;
    ...
    function buildBreadcrumb($category_id, $level = 0) {
      foreach ($this->_data as $parent => $categories) {
        if (isset($categories[$category_id])) {
          return ($parent == $this->root_category_id)
               ? $category_id
               : $this->buildBreadcrumb($parent, $level+1) . $this->breadcrumb_separator . $category_id;
        }
      }
 ...
 */
    ...
    function getDescendants($category_id, &$results = []) {
      foreach (array_keys($this->_data[$category_id]) as $id) {
        $results[] = $id;
        $this->getDescendants($id, $results);
      }
 ...
 */
    ...
    function setFollowCategoryPath($follow_cpath) {
      $this->follow_cpath = ($follow_cpath === true);
    }

...
*/
  ...
  const CATALOG_AUTOLOAD_PATH = DIR_FS_CACHE . 'catalog_autoload_index.cache';
  ...
  function tep_build_catalog_autoload_index() {
    ...
    if (!empty($class_files) && (!file_exists(CATALOG_AUTOLOAD_PATH) || is_writable(CATALOG_AUTOLOAD_PATH))) {
      // cache the class file index if we can
      file_put_contents(CATALOG_AUTOLOAD_PATH, serialize($class_files), LOCK_EX);
    }

    return $class_files;
  }

  function tep_load_autoload_index($path) {
    if (file_exists($path)) {
  ...
  function tep_autoload_catalog($class) {
    ...
    if (!isset($class_files)) {
      ...
      $class_files = tep_load_autoload_index(CATALOG_AUTOLOAD_PATH)
